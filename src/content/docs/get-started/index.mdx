...
Kùzu's Java client library is available on [Maven Central](https://central.sonatype.com/artifact/com.kuzudb/kuzu). You can add the following snippet to your `pom.xml` to get it installed:
...
For Groovy DSL:
```grovy
implementation 'com.kuzudb:kuzu:0.6.0'
```
...
public class Main {
    public static void main(String[] args) throws ObjectRefDestroyedException {
        ...
        // Load data.
        QueryResult r1 = conn.query("COPY User FROM 'src/main/resources/user.csv'");
        QueryResult r2 = conn.query("COPY City FROM 'src/main/resources/city.csv'");
        QueryResult r3 = conn.query("COPY Follows FROM 'src/main/resources/follows.csv'");
        QueryResult r4 = conn.query("COPY LivesIn FROM 'src/main/resources/lives-in.csv'");

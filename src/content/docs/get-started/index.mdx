...
Initialize a new project using Swift Package Manager:

```bash
mkdir kuzu-swift-get-started
swift package init --type=executable
```

Change your `Package.swift` file to include the following:
...
Next, replace the contents of `Sources/kuzu-swift-get-started/main.swift` with the following code:
...
Copy the built executable to the data directory:

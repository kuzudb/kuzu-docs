...
In addition to security, you may want to use parameters in your Cypher queries for greater flexibility
and dynamically adding data to your graph.
...
to the query, so that the same query can be run with different values.
...
Parameterized variables in Cypher are marked using the `$` symbol. The following query asks for only
persons who are between a minimum and maximum age in the database. The below example in Python shows

...
`cargo run` will take some time as it compiles Kuzu from scratch. The second 
and subsequent runs should be faster.
```bash
...
Run `create-db.rs` to create the Kuzu database and import the CSV files:
```bash
$ cargo run --bin create-db
...
This is because the data has already been imported into the Kuzu database.
:::
...
<Tabs>
<TabItem label="bin/create-db.rs">

...
- **userId** (INT64): This is an unique id which is used to sort and find users. 
...
- **postId** (INT64): This is an unique id which is used to sort and find posts.
...
```Rust
    ...
    conn.query("CREATE NODE TABLE Post(postId INT64 PRIMARY KEY, post_date DATE, like_count INT64, retweet_count INT64)")?;
...
```
Storing the result in the variable `result`. Which we can then access the content by using code snippet like this:
```Rust
    for row in result {
        let row_str = row.iter()
            .map(|col| col.to_string())  // Convert each column to a string
...
```Rust
    let result = conn.query("MATCH (u1:User)-[f:FOLLOWS]->(u2:User) RETURN u2.username, count(u2) as follower_count")?;
    for row in result {
        let row_str = row.iter()
            .map(|col| col.to_string())  // Convert each column to a string
            .collect::<Vec<String>>()    // Collect into a Vec<String>

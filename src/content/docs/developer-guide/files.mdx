...
| Lock file | `example.lock` |
| Write-ahead log file | `example.wal`  |
| Shadow file | `example.shadow` |
| Spillover file | `example.tmp` |
...
It is removed when the Kuzu process exits.
...
This is a **temporary file** that is created to help run checkpoint operations.

#### Spillover file

This is a **temporary file** used to spill intermediate memory data during query execution.
...
By default, the file is stored in the current working directory from where the CLI is started.
You can configure a different location for the history file using the `--path_history` or `-p` flags.
See `kuzu --help` for more details.

...
:::caution[Note]
Kuzu uses the WAL and shadow files to recover committed but uncheckpointed changes if the process running the database crashes. When deleting a kuzu database file (e.g. when you wish to create a fresh database instance at the same path), please also delete the WAL and shadow files if they exist as they serve no purpose for future databases.
:::

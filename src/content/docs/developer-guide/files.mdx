...
:::caution[Note]
Kuzu uses `.wal` and `.shadow` files to recover committed but uncheckpointed changes if the process running the database crashes. When deleting a Kuzu database file (e.g. when you wish to create a fresh database instance at the same path), you should also delete the corresponding `.wal` and `.shadow` files, if any. Loading a new (empty) Kuzu database with any leftover temporary files will result in an error.
:::

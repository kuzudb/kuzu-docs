...
If the file format cannot be inferred from the extension, or if users need to override the default sniffing behaviour, the file_format option can be used.

For example, to load a CSV file with a `.tsv` extension, the user must explicitly specify the file format using the `file_format` option, as shown below:
```

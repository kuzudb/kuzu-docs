...
- for parquet, pandas, polars & pyarrow, column names and data types are always available in the data source.
...
- for json, we use keys as column names, and infer a common data type from all values under the key.

To enforce specific column names and data types when readi, you can use the `LOAD WITH HEADERS (<name> <dataType>, ...) FROM ...` syntax.

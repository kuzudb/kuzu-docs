...
defined through `CREATE NODE TABLE` and `CREATE REL TABLE` commands.
...
You can update the schema of a table using the `ALTER TABLE` command.
...
You can drop tables from the database with the `DROP TABLE` command

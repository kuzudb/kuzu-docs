...
| `WARNING_LIMIT` | Maximum number of warnings that can be stored in a single connection. Currently only the warnings related to [malformed CSV lines](/import/csv#ignoring-erroneous-rows) and [malformed JSON lines](/import/copy-from-json#ignoring-erroneous-rows) are stored if `ignore_errors` parameter is set to true in `COPY FROM` and `LOAD FROM` statements. | 8192        |

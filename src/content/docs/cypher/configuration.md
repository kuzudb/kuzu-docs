...
| `WARNING_LIMIT` | Maximum number of warnings that can be stored in a single connection. Currently only the [warnings](/import#warnings-table-inspect-skipped-rows) related to erroneous rows are stored if `ignore_errors` parameter is set to true in `COPY FROM` and `LOAD FROM` statements. | 8192        |

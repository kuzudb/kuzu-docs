...
same type. Different from `STRUCT`, `MAP` doesn't require the same key to be present in each row.
Therefore, `MAP` is more suitable when the schema is not pre-determined.
...
and
```cypher
CREATE SEQUENCE Person_id_serial;
CREATE NODE TABLE table_name(id INT_64 default nextval('Person_id_serial'), PRIMARY KEY(id));
```

...
You can use `SERIAL` also to generate unique identifiers as properties in a relationship table.
Here's an example:
...
`RELATIONSHIP` is a logical type. Internally, `RELATIONSHIP` is processed as `STRUCT` type. A `RELATIONSHIP` always contains a
...
Here's how to return `RELATIONSHIP` column:
```cypher
...
`RECURSIVE RELATIONSHIP` is a logical type. Internally, `RECURSIVE RELATIONSHIP` is processed as `STRUCT` type, more specifically, a
`STRUCT{LIST[NODE], LIST[RELATIONSHIP]}`. A `RECURSIVE RELATIONSHIP` always contains a nodes field with the key `_NODES` and a
relationships field with the key `_RELS`.

#### Return `RECURSIVE RELATIONSHIP` column
```cypher

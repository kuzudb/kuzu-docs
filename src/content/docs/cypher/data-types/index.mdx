...
You can use `SERIAL` or `SEQUENCE` (a generalization of `SERIAL`) to generate unique values that
can serve as primary keys for your node tables.

You can use `SERIAL` to create a sequence as shown below:
```cypher
CREATE NODE TABLE Person(id SERIAL, PRIMARY KEY(id));
```
Alternatively, you can create a `SEQUENCE` using DDL and use it as a default primary key value
for a column.
...
See the documentation on [DDL](/cypher/data-definition/create#create-sequence) for a full list
of options for creating a `SEQUENCE`.

...
Error: Binder exception: Expression Alice has data type STRING but expected INT64. Implicit cast is not supported.
```
:::
...
```cypher
RETURN CAST([3,4,12,11], 'INT64[4]') as output;
```
...
```cypher
RETURN CAST([[5,2,1],[2,3],[15,64,74]], 'INT64[][3]') as output;
```
```
┌────────────────────────────┐
│ output                     │
...
```cypher
UNWIND CAST([[1,2,3],[3],[4,5]], 'INT64[][3]') AS x
UNWIND x AS y
RETURN y;
```
```

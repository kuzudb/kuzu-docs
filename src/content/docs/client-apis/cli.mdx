...
To interrupt a running query, use `Ctrl + C` in CLI. Note: We currently don't support interrupting a
running `COPY` statement.
...
## Query progress
![](./progress_bar.gif)
You can view the progress of a query by enabling the progress bar. To enable the progress bar, use the following Cypher command:

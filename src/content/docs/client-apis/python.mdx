...
- **`COPY FROM` or `LOAD FROM` with a Pandas or Polars DataFrame, or PyArrow Table**
  ...
  calling thread, and thus the dataframe isn't in scope for the executing thread. If you wish to reference dataframes when using the async API, please [pass them as query parameters](/import/copy-from-dataframe#query-parameter).

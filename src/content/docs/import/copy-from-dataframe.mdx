...
There are two different ways in Kuzu to reference dataframes:
...
This is the most reliable way of referencing a dataframe.

### Referencing by Name

The dataframe can also be referenced by name:
...
This method requires the dataframe to be a local variable that is in scope at the call to `conn.execute()`. For example, the code below will not work:
...
Due to the above limitations, we generally do not recommend using this method to reference dataframes. Please [pass the dataframes as parameters](#query-parameter) instead:
...
For more examples, refer to the [examples](#examples) on this page.

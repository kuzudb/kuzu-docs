...
Kùzu will spill some of this data to a `.tmp` file in the database directory.
...
### Warnings table: Inspect skipped rows
...
While the `IGNORE_ERRORS` option can be specified for all file import sources, the set of errors that can be skipped can be different. Below is a table that shows the errors that are skippable by each source.

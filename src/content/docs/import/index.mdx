...
When bulk inserting into a relationship table with multiple from-to pairs, users need to specify which child table to insert through `from, to` options, e.g.
```sql

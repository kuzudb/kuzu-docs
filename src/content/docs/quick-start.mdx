...
Once you have Kùzu's CLI or your desired client library installed, you can define a graph schema and
begin inserting data to the database using the Cypher query language. The example below uses a
...
Navigate to your respective client library language's tab as required to see the relevant code.

Because Kùzu is an embedded database, you can simply import `kuzu` and start interacting
with the database in your desired programming language. Cypher queries can be passed as strings
to the `execute` (or equivalent) method in the language's client library.
...
In Python, you can also use the [Pandas](https://pandas.pydata.org/) library to work with the
results of a Cypher query. This assumes that `pandas` is installed in your environment.
...
[Polars](https://pola.rs/) is another popular library for working with DataFrames in Python. This
assumes that `polars` is installed in your environment.
...
Arrow Tables in Python. This assumes that `pyarrow` is installed in your environment. Working with
Arrow tables is useful when you need to interoperate with other systems that use Arrow as a backend.
In fact, the `get_as_pl()` method shown above materializes a PyArrow table under the hood.
...
the cpp code file are all under the same directory:
...
The Kùzu C API shares the same so/dylib/dll+lib library files with the C++ API and can be used by

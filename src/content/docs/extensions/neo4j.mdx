...
* Multi-labelled nodes in Neo4j cannot be migrated as Kuzu doesn't support multi-labelled nodes.
* Nodes and relationships without any labels in Neo4j will be skipped during migration.
* Properties with Neo4j's data types that are not natively supported in Kuzu will be imported as a `STRING`.

...
```
- `HOST_NAME`: The domain name/IP address of a Neo4j server running on port 7474.
...
:::note
The `host_name` parameter must be a raw IP address or domain name without protocol prefixes (e.g., 192.168.1.50, www.neo4jdb.com, localhost).
Kuzu currently uses the neo4j default port 7474 and the port number cannot be customized.
:::

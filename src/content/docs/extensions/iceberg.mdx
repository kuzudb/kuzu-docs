...
```cypher
CREATE NODE TABLE people (name STRING, age INT64, PRIMARY KEY(age));
COPY student FROM 'kuzu/extension/iceberg/test/iceberg_tables/person_table' (file_format='iceberg')
```
:::note[Note]
...
Result:
```cypher
kuzu> CREATE NODE TABLE people (name STRING, age INT64, PRIMARY KEY(age));
┌─────────────────────────────────┐
│ result                          │
│ STRING                          │
├─────────────────────────────────┤
...
kuzu> COPY student FROM 'kuzu/extension/iceberg/test/iceberg_tables/people_table' (file_format='iceberg');
┌─────────────────────────────────────────────────┐
│ result                                          │
│ STRING                                          │
├─────────────────────────────────────────────────┤
│ 3 tuples have been copied to the people table.  │
└─────────────────────────────────────────────────┘
```

...
If the user attempts to install an extension that is already installed, the install command will skip downloading it from the official repository and will use the existing local copy instead.

To force Kuzu to download the extension from the official repository regardless of the presence of a local copy, users can use the `FORCE INSTALL` command followed by the extension name:
...
In contrast, self-compiled extensions must be loaded by providing the full file path to the compiled extension library.

For example:
To load the official `json` extension (after it has been installed), use:

```sql
load json;
```
However, for a self-compiled extension such as httpfs, you must specify the path to the compiled shared library:

```sql
#Assuming the user has compiled the httpfs extension and placed the binary under `extension/httpfs/build/`
...
Extensions can also be uninstalled after installation. To uninstall an extension, simply run the `UNINSTALL` command along with the extension name.

...
- `user`: Postgres username. Defaults to `postgres`.
- `password`: Postgres password. Defaults to an empty string.
...
### PostgreSQL to Kuzu types mapping
...
### Scanning PostgreSQL tables with Cypher
...
### Scanning PostgreSQL tables with SQL

You can use the `SQL_QUERY` function to execute arbitrary **read-only** SQL queries on any attached Postgres database
...
### Copying data from PostgreSQL tables
...
```
:::note[Note]
If you have attached to databases from different RDBMSs, say Postgres, DuckDB, and Sqlite, this call will clear the cache for _all_ of them.
:::

...
#### 5. Scan from arbitrary SQL queries

The `sql_query` table function allows users to execute any read SQL queries on any attached POSTGRESQL database and retrieve the result in Kuzu. The function takes in two parameters:
...
The below query shows an example to scan from a SQL query which filters out all people who is younger than 20:
```sql

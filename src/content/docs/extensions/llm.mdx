...
CALL CREATE_EMBEDDING(
    ...
    'model',
     dimensions,
    'region',
); // This function returns a LIST[FLOAT].
```
...
```sql
CALL CREATE_EMBEDDING("Hello world", "openai", "text-embedding-3-small");
...
# Note how the embeddings are created using the create_embedding function instead of a third party Python library.

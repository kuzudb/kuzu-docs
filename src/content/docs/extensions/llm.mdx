...
:::note[Note]
Ollama endpoint configuration works as follows. If an endpoint argument is provided it will be used to configure the endpoint. 
If an endpoint is not provided, and OLLAMA_URL is set, the OLLAMA_URL variable will be used. Otherwise, the default endpoint, 
`http://localhost:11434`, will be used.
:::

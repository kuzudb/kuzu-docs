...
:::note[Note]
Model parameters are dynamically validated at runtime. If an unsupported model is specified, the provider's API will return an appropriate error.
:::
---
...
:::note[Note]
The `ollama` provider requires no environment variables but does require the embedding model to be available at `http://localhost:11434`.
:::
---
...
Dimension configuration is supported for the following providers.

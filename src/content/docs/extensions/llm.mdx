...
The list of supported providers and their models is as follows:
...
- **dimensions** *(optional)*: The size of the embedding vector, if supported by the provider. Must be a positive integer.
- **region** *(optional)*: A region or endpoint hint, if required by the provider.

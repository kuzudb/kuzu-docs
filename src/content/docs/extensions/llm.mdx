...
| `ollama` | Optional: `OLLAMA_URL` |

:::note[Note]
Ollama endpoint configuration works as follows. If an endpoint is passed as an
argument it will be used. If an endpoint is not provided, and OLLAMA_URL
is set, the OLLAMA_URL variable will be used to configure the
endpoint. Otherwise, the default endpoint, `http://localhost:11434`, will be used.
:::

...
CALL CREATE_EMBEDDING(
    ...
    [option_name := option_value] // Optional parameters, if supported by the provider.
);
```
...
| Option | Description | 
|--------|-------------| 
| dimensions    | The size of the embedding vector, if supported by the provider. Currently only `'open-ai'`, `'google-vertex'`, and `'voyage-ai'` support this option. |
| region    | A region or endpoint, if supported by the provider. E.g., `'us-east-1'` for `'amazon-bedrock'`. |

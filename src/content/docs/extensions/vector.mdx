...
The `VECTOR` extension provides an on-disk [HNSW](https://en.wikipedia.org/wiki/Hierarchical_navigable_small_world)-based vector index for accelerating similarity search over float array columns in tables.
...
- **`FLOAT` Array Support Only**: Currently only supports 32-bit `FLOAT` type array columns. Support for 64-bit floats, i.e., `DOUBLE` type array columns is coming soon.
...
| `pu`                | Percentage of nodes sampled into the upper graph | 0.05 |
| `metric`       | Metric (distance computation) functions. Supported values are `cosine`, `l2`, `l2sq`, and `dotproduct` (dot_product). | cosine |
...
We use lazy evaluation to evaluate the predicate. The predicate is only evaluated when the projected graph is queried. As
a result, the project graph operation always runs instantly.

...
1. **Single Node Table Column Indexing**: Currently, you can only index a single column in a given node table.
2. **Immutable Index**: Once created, the index cannot be modified. You must drop and re-create the index to reflect changes in the underlying tables.
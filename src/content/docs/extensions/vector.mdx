...
:::note[Note]
A projected graph generated by `PROJECT_GRAPH_CYPHER` can currently be used for filtered vector index search only and cannot be used with the `algo` extension.
:::

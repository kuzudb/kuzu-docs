...
In the snippets below you will also see commented code that make use of the `CREATE_EMBEDDING` function from the `llm` extension.
This extension can remove the need to import third party libraries for embeddings. Check out the `llm` extension [here](/extensions/llm).
...
# The query can also be formed as follows:
# result = conn.execute(
#     """
#     CALL QUERY_VECTOR_INDEX(
#         'Book',

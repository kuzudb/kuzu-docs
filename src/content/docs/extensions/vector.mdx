...
Let's run some example search queries on our newly created vector index. You can again either use the `llm` extension to create the embeddings
...
### Filtered vector search with arbitrary cypher query

To run filtered search with arbitrary cypher query, first you need to create a projected graph using `project_graph_cypher`.
...
The cypher statement can contain arbitrary pattern matching. But in order to use the projected graph in vector filtered search, 
the return clause must contain a single node variable whose label equals to the table on which vector index is built.

:::note[Note]
A projected graph generated by `PROJECT_GRAPH_CYPHER` is limited to filtered vector search only and cannot be used in graph algorithms.
:::
...
```
You can replace `filtered_book` with `pearson_book` in the examples under filtered vector search section.
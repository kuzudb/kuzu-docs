...
Below is an example demonstrating how such a dataset can be created using the `sentence_transformers` library in Python to create the embeddings.
...
### Using the LLM extension

Instead of using `sentence_transformers`, you can use Kuzu's [`llm`](/extensions/llm) extension to directly create embeddings using Cypher.

To do so, you must load the extension at the beginning of the script:
...
To create the entries in the embedding table you can do the following:
...
You can also use the extension to generate the query embedding:

...
library, such as `sentence_transformers` in Python to create the embeddings. Alternatively, you can use Kuzu's [`embedding`](/extensions/embedding)
...
use_embedding_extension = True # Set to `False` to use sentence_transformers
...
if use_embedding_extension:
    conn.execute("INSTALL embedding; LOAD embedding;")
else:
...
Let's run some example search queries on our newly created vector index. We can again either use the `embedding` extension to create the embeddings

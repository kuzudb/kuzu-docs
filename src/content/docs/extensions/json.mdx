...
## JSON Type
KuzuDB supports `json` via the `JSON` logical type. The `JSON` logical type is interpreted as parsed JSON, 
rather than interpreted as `string`.
...
```cypher
RETURN to_json('{"name": "Alicia", "age": 28}');
```
Output:
```json
{"name":"Alicia","age":28}
...
```cypher
RETURN json_array(42, 'kuzu', NULL);
```
```json
[42,"kuzu",null]
```
...
```json
```
(merge with a NULL path would result in NULL path)
...
Integers may also be used in path, which represents index in JSON arrays.
If `path` is a `LIST`, the result will be a `LIST` of JSON.
...
```cypher
RETURN json_extract('{"Software": {"Database": ["duck", "kuzu"]}}', 'Software/Database/1');
```
Output:
```json
"kuzu"
...
```cypher
RETURN json_extract('{"Software": {"Database": ["duck", "kuzu"]}}', ['Software/Database/1', 'Software/Database/0']);
```
Output:
```json
["kuzu","duck"]
...
```cypher
RETURN json_extract('{"Software": {"Database": ["duck", "kuzu"]}}', 'Software/Musicplayer/1');
```
Output:
```json
```
(Extract from an nonexisted path results in empty `JSON` document)
...
```cypher
RETURN JSON_CONTAINS('{"name": "Alicia"}', '"Alicia"');
```
Output:
```json
True
...
```cypher
RETURN JSON_CONTAINS('{"age": 28}', '28');
```
Output:
```json
True
...
Gives the keys of the root `json` object. If the root is not an JSON object, returns an empty `list`.
...
Returns the structure of the `json` in Kuzu type notation. Will give smaller integer widths for smaller numbers.
...
```cypher
RETURN json_valid('{"name": "Alicia", "age": 28}');
```
Output:
```json
True

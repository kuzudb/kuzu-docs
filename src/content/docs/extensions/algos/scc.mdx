...
Due to the nature of DFS, `scc_kosaraju` can only run in single-threaded mode. In the majority of
cases, it's recommended to use `scc`, but Kosaraju's algorithm can be useful in the following cases:
...
`group_id` is assigned based on Kuzu'sinternal node offsets. Currently there is no way to assign `group_id` based on node properties.
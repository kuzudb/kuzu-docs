...
The first step to run a graph algorithm on a Kuzu database table is to create a projected subgraph.
...
A projected subgraph is evaluated _only_ when the algorithm is executed, so it can always
scan the most up-to-date data from the database.
:::
...
```cypher
CALL weakly_connected_component('Graph')
...
```cypher
CALL weakly_connected_component('Filtered_Graph')
RETURN group_id, collect(node.id);
```
```
┌──────────┬──────────────────┐

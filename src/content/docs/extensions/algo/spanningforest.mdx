...
connected component of a graph without forming cycles. This algorithm treats all edges as
undirected during processing. The result is a table of edges that belong to the spanning forest,
so isolated vertices are not included. Spanning forests are commonly used in
...
- `forest_id`: An id indicating the tree (component) the `rel` belongs to.

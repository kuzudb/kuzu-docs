...
- [Client APIs](#client-apis)
  - [C++ UDF](#c-udf)
  - [Converting Query Result to Arrow](#converting-query-result-to-arrow)
...
The integration with rel tables and more powerful compression schemes, e.g., constant compression, bit-packing, dictionary compression, will come soon.
...
In this release, we implemented unnest correlated subquery based on the paper (Unnesting Arbitrary Queries)[https://cs.emis.de/LNI/Proceedings/Proceedings241/383.pdf] and move to hash join based solution. The following plan will be generated for the above query.
...
See [map](../docs/cypher/data-types/map.md) for more informaiton.
...
In this release, converting to arrow arrays are now also available in Rust, [C](./../client-apis/c-api/query_result) (see `kuzu_query_result_get_arrow_schema` and `kuzu_query_result_get_next_arrow_chunk`), and [C++ apis](./../client-apis/cpp-api/query-result) (see `getArrowSchema` and `getNextArrowChunk`).

...
```
Note that for this query to run successfully, each relationship table in your database needs to have a `since` property defined.
That is suppose your database has `Follows` and `Likes` relationships, then the above query will bind `f` sometimes to `Follows`
and sometimes to `Likes` relationships and the `since` property needs to be defined on both `Follows` and `Likes` relationship
tables. A similar situation would exist when updating the property of a variable `x`, where `x` binds to multiple node tables.

...
We have moved to SQL-style casting function `cast(input, target type)`. 
```
RETURN cast(2.3, "INT8");
RETURN cast("[1,2,3]", "LIST[INT64]");
```
...
Kùzu now supports filtering intermediate node on 
```
Match p = (a:User)-[:Follows (r, n | WHERE n.age > 21)]->(b:User) RETURN p;
```

...
2 Intel Xeon Platinum 8175M CPUs, each of which has 48 physical cores cores. We used 300 GB of the 380GB total RAM as buffer pool.
...
`RETURN` clause now can return variable bindings that were used in the `DELETE` command. For example,
you can return the nodes that were deleted in previous DELETE command in the same Cypher query as follows: 
```
...
We have implemented a SQL-style single `cast` function `cast(input, target_type)` to cast values to different
types. The cast function will convert the `input` argument to the `target_type` as the 
cast of the input value to target type is defined. For example:
```
...
Since v 0.0.5 we have been supporting filtering the intermediate relationships that can bind to
recursive relationships, based on their properties of these intermediate relationships. 
With the current release, we also support filtering the intermediate nodes that are bound to recursive relationships.
...
the second variable `n` binds to the intermediate relationships. The `|`symbol can be followed with a `WHERE` clause
...
We have added support for count subqueries, which checks the number of matches for given pattern in the graph.
...
### Reduced Binary Size
We did With this release, we removed our Apache Arrow dependency, which significantly reduces oure binary size. 
Additionally, we now strip the shared library and CLI binaries of the symbols that are needed by our 
...
Stripping of our other libraries is a work in progress. 

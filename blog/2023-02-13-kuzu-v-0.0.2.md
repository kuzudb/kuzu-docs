...
new [`QueryResult.get_as_torch_geometric()`](../docs/client-apis/python-api/query-result.md#query_result.QueryResult.get_as_torch_geometric) function that 
...
Our [Python API](https://kuzudb.com/docs/client-apis/python-api/overview.html) now has a 
new [`QueryResult.get_as_networkx()`](../docs/client-apis/python-api/query-result.md#query_result.QueryResult.get_as_networkx) function that can convert query results
...
we infer that this is a parquet file and parse it so. See the details [here](../docs/cypher/../data-import/parquet-import.md).
...
2) Support of [regex pattern matching](../docs/cypher/expressions/pattern-matching.md) with strings. Kùzu now supports Cypher's `=~` operator for regex searches, which will return true if its pattern mathces the entire input string. For example: `RETURN 'abc' =~ '.*(b|d).*';`.

### CASE Expression
We've added [CASE](../docs/cypher/expressions/case-expression.md) for conditional expressions.
Two forms ([Simple Form](../docs/cypher/expressions/case-expression.md#simple-form) and [General Form](../docs/cypher/expressions/case-expression.md#general-form)) of CASE expression are supported.

### ALTER/DROP/SET/DELETE
We added [ALTER TABLE](../docs/cypher/ddl.md#alter-table) and [DROP TABLE](../docs/cypher/ddl.md#drop-table) DDL statements.
...
Besides schema level changes, you can change properties of existing nodes/rels with [SET](../docs/cypher/data-manipulation-clauses/set.md) statements, and remove existing nodes/rels with [DELETE](../docs/cypher/data-manipulation-clauses/delete.md) statements.

...
to evaluate LLMs with different prompting approaches. The set of techniques are too detailed to cover here but briefly include
...
So most work is on generating SQL.
...
*Important Future Work: Perform similar prompting studies for query languages of graph DBMSs.*: Given the new benchmarks
people are providing, the infrastructure for this type of evaluation may be available. At least for Cypher two immediate 
and previous benchmarks that can be repurposed, similar results
